...
from data.structs import BookCrossingInteractionGraph, InteractionGraph, MovielensInteractionGraph
...
from data.datareader import read_bx, read_movielens
from model.sparsenn import BookCrossingSparseNN, MovieLensSparseNN
...
flags.DEFINE_string("set_embedding_model", "mean", "The type of set embedding model to use")
...
flags.DEFINE_string("scoring_func", "dot_compress", "Type of scoring fun")
...
def batch_to_device_ml(user_feats, item_feats, neighbor_item_feats, device):
    user_ids, user_genders, user_ages, user_occs = user_feats
    movie_ids, movie_dates, movie_genres, movie_title_embeddings = item_feats
    neighbor_movie_ids, neighbor_movie_dates, neighbor_movie_genres, neighbor_movie_title_embeddings = neighbor_item_feats
    ...
    neighbor_movie_ids = neighbor_movie_ids.to(device, non_blocking=True)
    neighbor_movie_dates = neighbor_movie_dates.to(device, non_blocking=True)
    neighbor_movie_genres = neighbor_movie_genres.to(device, non_blocking=True)
    neighbor_movie_title_embeddings = neighbor_movie_title_embeddings.to(device, non_blocking=True)

    return (user_ids, user_genders, user_ages, user_occs), (movie_ids, movie_dates, movie_genres, movie_title_embeddings), (neighbor_movie_ids, neighbor_movie_dates, neighbor_movie_genres, neighbor_movie_title_embeddings)


def batch_to_device_bx(user_feats, item_feats, neighbor_item_feats, device):
    ...
    neighbor_book_ids, neighbor_book_authors, neighbor_book_dates, neighbor_book_publishers, neighbor_book_title_embeddings = neighbor_item_feats

    user_ids, user_locations, user_ages = user_ids.to(device, non_blocking=True), user_locations.to(device, non_blocking=True), user_ages.to(device, non_blocking=True)
    ...
    neighbor_book_ids = neighbor_book_ids.to(device, non_blocking=True)
    neighbor_book_authors = neighbor_book_authors.to(device, non_blocking=True)
    neighbor_book_dates = neighbor_book_dates.to(device, non_blocking=True)
    neighbor_book_publishers = neighbor_book_publishers.to(device, non_blocking=True)
    neighbor_book_title_embeddings = neighbor_book_title_embeddings.to(device, non_blocking=True)

    user_feats = (user_ids, user_locations, user_ages)
    item_feats = (book_ids, book_authors, book_dates, book_publishers, book_title_embeddings)
    neighbor_item_feats = (neighbor_book_ids, neighbor_book_authors, neighbor_book_dates, neighbor_book_publishers, neighbor_book_title_embeddings)

    return user_feats, item_feats, neighbor_item_feats
...
def train_ml():
    ...
    compact_adj = convert_to_compact_adj(ml_graph)

    bias_dist = softmax_degree_prob if FLAGS.sampling_bias else uniform_degree_prob
    bias_func = compact_adj.create_multinomial_sampler(bias_dist, ml_graph.start_item_id, temp=FLAGS.sampling_softmax_temp, order=FLAGS.order)
    ...
    ml_collator_train = MovieLensCollator(ml_graph, one_hop_runner, mode='train', num_neg_samples=FLAGS.num_negatives, fanouts=FLAGS.fanouts)
    ml_dataloader_train = DataLoader(ml_dataset_train, batch_size=FLAGS.batch_size, collate_fn=ml_collator_train, worker_init_fn=init_fn, num_workers=FLAGS.num_workers, shuffle=True, pin_memory=True)


    ml_dataset_val = MovieLensDataset(ml_graph, mode='test')
    ml_collator_val = MovieLensCollator(ml_graph, one_hop_runner, mode='test', fanouts=FLAGS.fanouts)
    ml_dataloader_val = DataLoader(ml_dataset_val, batch_size=FLAGS.batch_size, collate_fn=ml_collator_val, worker_init_fn=init_fn, num_workers=FLAGS.num_workers, pin_memory=True)


    ml_inference_users_dataset = MovieLensInferenceUsersDataset(ml_graph)
    ml_users_collator = MovieLensUsersCollator(ml_graph, one_hop_runner, fanouts=FLAGS.fanouts)
    ml_inference_users_dataloader = DataLoader(ml_inference_users_dataset, batch_size=FLAGS.item_inference_batch_size, collate_fn=ml_users_collator, worker_init_fn=init_fn, num_workers=0, pin_memory=True)

    ml_inference_items_dataset = MovieLensInferenceItemsDataset(ml_graph)
    ml_items_collator = MovieLensItemsCollator(ml_graph)
    ml_inference_items_dataloader = DataLoader(ml_inference_items_dataset, batch_size=FLAGS.item_inference_batch_size, collate_fn=ml_items_collator, worker_init_fn=init_fn, num_workers=0, pin_memory=True)
    ...
    ml_sparseNN = MovieLensSparseNN(
        ...
        set_embedding_model=FLAGS.set_embedding_model,
        scoring_model=FLAGS.scoring_func
    ).to(device)
    ...
    for epoch in range(FLAGS.epochs):
        for i, (user_feats, item_feats, neighbor_item_feats) in enumerate(tqdm(ml_dataloader_train)):
            user_feats, item_feats, neighbor_item_feats = batch_to_device_ml(user_feats, item_feats, neighbor_item_feats, device)
            optimizer.zero_grad()
            user_ids, user_genders, user_ages, user_occs = user_feats
            movie_ids, movie_dates, movie_genres, movie_title_embeddings = item_feats
            neighbor_movie_ids, neighbor_movie_dates, neighbor_movie_genres, neighbor_movie_title_embeddings = neighbor_item_feats
            ...
            scores = ml_sparseNN(user_ids, user_genders, user_ages, user_occs, 
                                 movie_ids, movie_dates, movie_genres, movie_title_embeddings,
                                 neighbor_movie_ids, neighbor_movie_dates, neighbor_movie_genres, neighbor_movie_title_embeddings).flatten()
...
def train_bx():
    ...
    compact_adj = convert_to_compact_adj(bx_graph)

    bias_dist = softmax_degree_prob if FLAGS.sampling_bias else uniform_degree_prob
    bias_func = compact_adj.create_multinomial_sampler(bias_dist, bx_graph.start_item_id, temp=FLAGS.sampling_softmax_temp, order=FLAGS.order)
    one_hop_runner = gttf(compact_adj, bias_func=bias_func, acc_func=accumulate_one_hop_neighbors)

    init_fn = InitFn(FLAGS.seed, FLAGS.num_workers)
    
    bx_dataset_train = BookCrossingDataset(bx_graph, mode='train')
    bx_collator_train = BookCrossingCollator(bx_graph, one_hop_runner, mode='train', num_neg_samples=FLAGS.num_negatives, fanouts=FLAGS.fanouts)
    bx_dataloader_train = DataLoader(bx_dataset_train, batch_size=FLAGS.batch_size, collate_fn=bx_collator_train, worker_init_fn=init_fn, num_workers=FLAGS.num_workers, shuffle=True, pin_memory=True)


    bx_dataset_val = BookCrossingDataset(bx_graph, mode='test')
    bx_collator_val = BookCrossingCollator(bx_graph, one_hop_runner, mode='test', fanouts=FLAGS.fanouts)
    bx_dataloader_val = DataLoader(bx_dataset_val, batch_size=FLAGS.batch_size, collate_fn=bx_collator_val, worker_init_fn=init_fn, num_workers=FLAGS.num_workers, pin_memory=True)


    bx_inference_users_dataset = BookCrossingInferenceUsersDataset(bx_graph)
    bx_users_collator = BookCrossingUsersCollator(bx_graph, one_hop_runner, fanouts=FLAGS.fanouts)
    bx_inference_users_dataloader = DataLoader(bx_inference_users_dataset, batch_size=FLAGS.item_inference_batch_size, collate_fn=bx_users_collator, worker_init_fn=init_fn, num_workers=0, pin_memory=True)
    ...
    bx_inference_items_dataloader = DataLoader(bx_inference_items_dataset, batch_size=FLAGS.item_inference_batch_size, collate_fn=bx_items_collator, worker_init_fn=init_fn, num_workers=0, pin_memory=True)
    
    device = torch.device(0) if (torch.cuda.is_available() and FLAGS.device == 'gpu') else torch.device('cpu')

    bx_sparseNN = BookCrossingSparseNN(
        num_user_ids=len(user_data), num_user_locations=num_user_locations, num_user_ages=num_user_ages,
        num_book_ids=len(item_data), num_book_authors=num_book_authors, num_book_dates=num_book_dates, num_book_publishers=num_book_publishers,
        feat_embed_dim=96,
        dense_feat_embed_dim=384,
        set_hidden_dims=[256, 128],
        item_embed_hidden_dims=[256, 128],
        scoring_function_hidden_dims=[128, 64],
        set_embed_dim=128,
        seq_len=FLAGS.fanouts[0],
    ...
    corr_cold_hr, corr_warm_hr, corr_cold_ndcg, corr_warm_ndcg = 0, 0, 0, 0
    for epoch in range(FLAGS.epochs):
        for i, (user_feats, item_feats, neighbor_item_feats) in enumerate(tqdm(bx_dataloader_train)):
            user_feats, item_feats, neighbor_item_feats = batch_to_device_bx(user_feats, item_feats, neighbor_item_feats, device)
            optimizer.zero_grad()
            user_ids, user_locations, user_ages = user_feats
            book_ids, book_authors, book_dates, book_publishers, book_title_embeddings = item_feats
            neighbor_book_ids, neighbor_book_authors, neighbor_book_dates, neighbor_book_publishers, neighbor_book_title_embeddings = neighbor_item_feats
            ...
            scores = bx_sparseNN(user_ids, user_locations, user_ages,
                                 book_ids, book_authors, book_dates, book_publishers, book_title_embeddings,
                                 neighbor_book_ids, neighbor_book_authors, neighbor_book_dates, neighbor_book_publishers, neighbor_book_title_embeddings).flatten()
...
def inference(model, 
    ...
    def compute_all_set_representations_ml():
        set_representations = torch.zeros((len(user_loader.dataset), model.set_embed_dim), dtype=torch.float32, device=device)
        for _, (user_feats, neighbor_item_feats) in enumerate(user_loader):
            ...
            user_feats = model.user_feature_model(user_ids, user_genders, user_ages, user_occs)
            neighbor_item_feats = model.item_feature_model(neighbor_movie_ids, neighbor_movie_dates, neighbor_movie_genres, neighbor_movie_title_embeddings)
            set_embeds_precomputed = model.set_embedding_model(user_feats, neighbor_item_feats.view(user_feats.shape[0], -1, neighbor_item_feats.shape[1]))      

            set_representations[user_ids, :] = set_embeds_precomputed
        
        return set_representations


    def compute_all_item_representations_ml():
        item_representations = torch.zeros((len(item_loader.dataset), model.set_embed_dim), dtype=torch.float32, device=device)
        for _, (movie_ids, zero_indexed_movie_ids, movie_dates, movie_genres, movie_title_embeddings) in enumerate(item_loader):
            ...
            item_feats = model.item_feature_model(zero_indexed_movie_ids, movie_dates, movie_genres, movie_title_embeddings)
            ...
            for i in range(len(item_embeddings_precomputed)):
                item_representations[item_loader.dataset.item_reindexer[movie_ids[i].item()], :] = item_embeddings_precomputed[i]

        return item_representations

    def compute_all_set_representations_bx():
        set_representations = torch.zeros((len(user_loader.dataset), model.set_embed_dim), dtype=torch.float32, device=device)
        for _, (user_feats, neighbor_item_feats) in enumerate(user_loader):
            user_ids, user_locations, user_ages = user_feats
            neighbor_book_ids, neighbor_book_authors, neighbor_book_dates, neighbor_book_publishers, neighbor_book_title_embeddings = neighbor_item_feats
            ...
            user_feats = model.user_feature_model(user_ids, user_locations, user_ages)
            neighbor_item_feats = model.item_feature_model(neighbor_book_ids, neighbor_book_authors, neighbor_book_dates, neighbor_book_publishers, neighbor_book_title_embeddings)
            set_embeds_precomputed = model.set_embedding_model(user_feats, neighbor_item_feats.view(user_feats.shape[0], -1, neighbor_item_feats.shape[1]))      
    ...
    def compute_all_item_representations_bx():
        ...
        for _, (book_ids, zero_indexed_book_ids, book_authors, book_dates, book_publishers, book_title_embeddings) in enumerate(item_loader):
            ...
            item_feats = model.item_feature_model(zero_indexed_book_ids, book_authors, book_dates, book_publishers, book_title_embeddings)
            item_embeddings_precomputed = model.item_embedding_model(item_feats)

            book_ids = book_ids.cpu()
            for i in range(len(item_embeddings_precomputed)):
                item_representations[item_loader.dataset.item_reindexer[book_ids[i].item()], :] = item_embeddings_precomputed[i]
    ...
    if dataset == 'ml':
        set_representations, item_representations = compute_all_set_representations_ml(), compute_all_item_representations_ml()
    else:
        set_representations, item_representations = compute_all_set_representations_bx(), compute_all_item_representations_bx()
    ...
    for _, (true_edges, edges_to_score, offsets) in enumerate(tqdm(val_loader)):
        for i, true_edge in enumerate(true_edges):
            ...
            precomputed_set_embeddings = set_representations[all_edges_u]
            precomputed_movie_embeddings = item_representations[candidate_items]
            scores = model(set_embeddings_precomputed=precomputed_set_embeddings, item_embeddings_precomputed=precomputed_movie_embeddings).flatten()
