# Duplicated from https://github.com/Elanmarkowitz/KGCompletionGNN

import numpy as np


class LeftContiguousCSR:
    def __init__(self, indptr: np.ndarray, degrees: np.ndarray, data: np.ndarray, timestamps: np.ndarray):
        self.indptr = indptr
    ...
    def get_timestamps(self, i):
        start_ind = self.indptr[i]
        end_ind = start_ind + self.degrees[i]
        return self.timestamps[start_ind:end_ind]
