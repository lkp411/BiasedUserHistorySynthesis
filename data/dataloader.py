...
class MovieLensCollator:
    def __init__(self, 
                 igraph: MovielensInteractionGraph, 
    ...
    def _fetch_item_data(self, items):
        item_dates = np.array([self.item_data[movie_id]['date'] for movie_id in items])
        item_genres = np.array([self.item_data[movie_id]['genres'] for movie_id in items])
        item_title_embeddings = np.array([self.item_data[movie_id]['title_embedding'] for movie_id in items])
        item_ids = items - len(self.user_data)

        return (torch.as_tensor(item_ids, dtype=torch.int64), torch.as_tensor(item_dates, dtype=torch.int64), 
        torch.as_tensor(item_genres, dtype=torch.float32), torch.as_tensor(item_title_embeddings, dtype=torch.float32))
    ...
    def __call__(self, positive_edges):
        ...
        edges = np.array(positive_edges + negative_edges) #np.repeat(np.array(positive_edges), 41, axis=0)# + negative_edges)

        one_hop_edges = self.runner(edges[:, 0], self.fanouts, edge_items=edges[:, 1], filter=True)

        user_feats = self._fetch_user_data(edges[:, 0])
        original_edge_item_feats = self._fetch_item_data(edges[:, 1])
        ...
        return user_feats, original_edge_item_feats, one_hop_item_feats
        
        

class MovieLensInferenceItemsDataset(Dataset):
    def __init__(self, igraph : MovielensInteractionGraph, mode='validation') -> None:
...
class MovieLensUsersCollator(MovieLensCollator):
    def __init__(self, igraph: MovielensInteractionGraph, runner: Callable[[np.ndarray, List[int], np.ndarray, bool], np.ndarray], fanouts=None) -> None:
        super().__init__(igraph, runner, mode='test', fanouts=fanouts)

    def __call__(self, batch):
        batch = np.array(batch)

        user_feats = self._fetch_user_data(batch)
        one_hop_edges = self.runner(batch, self.fanouts, edge_items=None, filter=False)
        unique_one_hop_items, unique_one_hop_idxs = np.unique(one_hop_edges[:, 1], return_inverse=True)
        unique_one_hop_idxs = torch.as_tensor(unique_one_hop_idxs)
        one_hop_ids, one_hop_dates, one_hop_genres, one_hop_title_embeddings = self._fetch_item_data(unique_one_hop_items)
        one_hop_item_feats = one_hop_ids[unique_one_hop_idxs], one_hop_dates[unique_one_hop_idxs], one_hop_genres[unique_one_hop_idxs], one_hop_title_embeddings[unique_one_hop_idxs]
...
class BookCrossingCollator:
    def __init__(self, igraph: BookCrossingInteractionGraph, 
                 runner: Callable[[np.ndarray, List[int], np.ndarray, bool], np.ndarray],  
                 mode: str, 
                 num_neg_samples=1, 
                 fanouts=None,
                 load_negatives=True) -> None:
        ...
        if self.mode != 'train' and load_negatives:
    ...
    def _fetch_user_data(self, users):
        user_feats = np.array([(user_id, self.user_data[user_id]['location'], self.user_data[user_id]['age']) for user_id in users])
        user_ids = torch.as_tensor(user_feats[:, 0], dtype=torch.int64)
        user_locations = torch.as_tensor(user_feats[:, 1], dtype=torch.int64)
        user_ages = torch.as_tensor(user_feats[:, 2], dtype=torch.int64)

        return user_ids, user_locations, user_ages

    def _fetch_item_data(self, items):
        item_feats = np.array([(book_id, self.item_data[book_id]['author'], self.item_data[book_id]['date'], self.item_data[book_id]['publisher']) for book_id in items])
        book_ids = torch.as_tensor(item_feats[:, 0], dtype=torch.int64) - len(self.user_data)
        book_authors = torch.as_tensor(item_feats[:, 1], dtype=torch.int64)
        book_dates = torch.as_tensor(item_feats[:, 2], dtype=torch.int64)
        book_publishers = torch.as_tensor(item_feats[:, 3], dtype=torch.int64)
        book_title_embeddings = np.array([self.item_data[book_id]['title_embedding'] for book_id in items])

        return (torch.as_tensor(book_ids, dtype=torch.int64), torch.as_tensor(book_authors, dtype=torch.int64), 
        torch.as_tensor(book_dates, dtype=torch.int64), torch.as_tensor(book_publishers, dtype=torch.int64), torch.as_tensor(book_title_embeddings, dtype=torch.float32))
    ...
    def __call__(self, positive_edges):
        ...
        negative_edges = self._generate_in_and_oob_negatives(positive_edges)
        edges = np.array(positive_edges + negative_edges)
        one_hop_edges = self.runner(edges[:, 0], self.fanouts, edge_items=edges[:, 1], filter=False)
        ...
        unique_one_hop_items, unique_one_hop_idxs = np.unique(one_hop_edges[:, 1], return_inverse=True)
        unique_one_hop_idxs = torch.as_tensor(unique_one_hop_idxs)
        one_hop_ids, one_hop_authors, one_hop_dates, one_hop_publishers, one_hop_title_embeddings = self._fetch_item_data(unique_one_hop_items)
        one_hop_item_feats = one_hop_ids[unique_one_hop_idxs], one_hop_authors[unique_one_hop_idxs], \
                             one_hop_dates[unique_one_hop_idxs], one_hop_publishers[unique_one_hop_idxs], one_hop_title_embeddings[unique_one_hop_idxs]
...
class BookCrossingItemsCollator:
    ...
    def __call__(self, batch):
        item_feats = np.array([(book_id, self.item_data[book_id]['author'], self.item_data[book_id]['date'], self.item_data[book_id]['publisher']) for book_id in batch])
        book_ids = torch.as_tensor(item_feats[:, 0], dtype=torch.int64)
        zero_indexed_book_ids = book_ids - len(self.igraph.user_data)
        book_authors = torch.as_tensor(item_feats[:, 1], dtype=torch.int64)
        book_dates = torch.as_tensor(item_feats[:, 2], dtype=torch.int64)
        book_publishers = torch.as_tensor(item_feats[:, 3], dtype=torch.int64)
        book_title_embeddings = torch.as_tensor(np.array([self.item_data[book_id]['title_embedding'] for book_id in batch]))
...
class BookCrossingUsersCollator(BookCrossingCollator):
    def __init__(self, igraph: MovielensInteractionGraph, runner: Callable[[np.ndarray, List[int], np.ndarray, bool], np.ndarray], fanouts=None) -> None:
        super().__init__(igraph, runner, mode='test', fanouts=fanouts, load_negatives=False)
